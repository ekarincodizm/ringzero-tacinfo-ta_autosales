...
$qry_select = pg_query("select * from \"installGas\" where \"gasInvoiceNo\" = '$gasInvoiceNo'");
...
// เพิ่มข้อมูลหลัก
$qry_installGas = "insert into \"installGas\"(\"gasInvoiceNo\", \"gasInvoiceDate\", \"wh_id\", \"Vender\", \"payDate\", \"cash_amount\", \"cheque_amount\",
							\"chequeNO\", \"BankCode\", \"payerCheque\", \"sum_amount\", \"discount\", \"payTrue\", \"doerID\", \"doerStamp\")
						values('$gasInvoiceNo', '$gasInvoiceDate', '$wh_id', $storeName, $payDate, '$cash_amount', '$cheque_amount',
							$chequeNO, $BankCode, $payerCheque, '$sum_amount', '$discount', '$payTrue', '$iduser', '$nowDateTime')";

if(!$res=@pg_query($qry_installGas)){
	$Error .= "บันทึก ไม่สำเร็จ  $qry_installGas \n";
	$status++;
}
...
for($i=1; $i<=$rowDetail; $i++)
{
	...
	$received_date[$i] = checknull($received_date[$i]);
	$res_id[$i] = checknull($res_id[$i]);
	$eng_cert[$i] = checknull($eng_cert[$i]);
	
	$qry_installGasDetail = "insert into \"installGasDetail\"(\"gasInvoiceNo\", \"auto_id_CarMove\", \"gas_install_date\", \"car_idno\", \"car_name\", \"car_num\", \"eng_cert\",
								\"send_gas_date\", \"received_date\", \"res_id\", \"gas_deatils\", \"install_amount\", \"oil_amount\")
							values('$gasInvoiceNo', '$auto_id_CarMove[$i]', '$gas_install_date[$i]', '$car_idno[$i]', '$car_name[$i]', '$car_num[$i]', $eng_cert[$i],
								'$send_gas_date[$i]', $received_date[$i], $res_id[$i], '$gas_deatils[$i]', '$install_amount[$i]', '$oil_amount[$i]')";
	
	if(!$res=@pg_query($qry_installGasDetail)){
		$Error .= "บันทึก ไม่สำเร็จ  $qry_installGasDetail \n";
		$status++;
	}
}
...
if($status == 0){
	...
	echo "บันทึกสำเร็จ";
	echo "</center>";
	echo "<meta http-equiv='refresh' content='2; URL=frm_save_detail_install_gas.php'>";
	...
	echo $data;
	echo "<meta http-equiv='refresh' content='5; URL=frm_save_detail_install_gas.php'>";
