...
$pid=$_GET["pid"];
$qry_p=pg_query("select * from \"Products\" where product_id='$pid' ");
...
<div>
  <div style="float:left"><b>Edit Product </b></div><div style="float:right; "><button style="width:150px;" onclick="window.location='product.php'">Back to list product</button></div><br />
  <div>
    <table width="578" border="0" cellpadding="2">
  <tr >
    <td colspan="4" style="text-align:right;">Product Name</td>
    <td width="456"><input type="text" name="p_name" id="p_name" style="width:300px;" value="<?php echo $res_p["name"]; ?>"  /></td>
    </tr>
  <tr>
	<td colspan="4" style="text-align:right;">Product Tables</td>
    <td width="456">
	<select name="cb_product" id="cb_product">
		<option value="">กรุณาเลือกตาราง</option>
							<?php 
								$qry_table = pg_query("select * from \"LinksProducts\" order by table_id");
								while($res = pg_fetch_array($qry_table)){
									$table_id = $res['table_id'];
									$table_name = $res['table_name'];								
							?>
								<option value="<?php echo "$table_name"; ?>" <?php if($table_name == $res_p["link_table"]) echo "selected"; ?>><?php echo "$table_name" ; ?></option>
							<?php
								}
							?>
	</select>
...
$('#btnSave').click(function(){
	...
	//alert(txttable);
		...
		if($('#p_name').val() == ""){
			msg += "กรุณาระบุ Product Name \n";
			chk++;
		}
		if($('#p_costprice').val() == ""){
			msg += "กรุณาระบุ  cost price \n";
			chk++;
		}
		if($('#p_saleprice').val() == ""){
			msg += "กรุณาระบุ sale price \n";
			chk++;
		}
		if(!$('input[id=usevat]:checked').val()){
			msg += "กรุณาระบุ use vat \n";
			chk++;
		}
		if(!$('input[id=type_rec]:checked').val()){
			msg += "กรุณาระบุ type rec \n";
			chk++;
		}
