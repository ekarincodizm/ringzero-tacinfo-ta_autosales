...
$po_id = pg_escape_string($_GET['po_id']);
...
$('#appv').click(function(){
	...
	$.post('process_print_log.php',{
		remark : $('#remark').val(),
		po_id : '<?php echo $po_id; ?>'
	},function(data){
		if(data == 1){
			alert('บันทึกข้อมูลเรียบร้อยแล้ว');
			ShowPrint('<?php echo $po_id; ?>','<?php echo $type; ?>')
		}else{
			alert(data);
			location.reload();
		}
...
function ShowPrint(id,type){
	if(type == "M"){
		var pdf_form = 'po_buy_mat_pdf.php';
	}else if(type == "C"){
		var pdf_form = 'po_buy_pdf.php';
	}
    $('body').append('<div id="divdialogprint"></div>');
	$('#divdialogprint').html("<div style=\"text-align:center\"><br /><br /><input type=\"button\" name=\"btnPrint\" id=\"btnPrint\" value=\"พิมพ์เอกสาร\" onclick=\"javascript:window.open('../report/"+pdf_form+"?po_id="+ id +"','po_id4343423','toolbar=no,menubar=no,resizable=yes,scrollbars=yes,status=no,location=no,width=800,height=600'); javascript:location.reload();\"></div>");
