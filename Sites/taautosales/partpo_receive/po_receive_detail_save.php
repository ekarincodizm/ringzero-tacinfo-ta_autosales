<?php
	...
	// var_dump($parts_received_details_array);
	// pg_query("ROLLBACK");
	// exit;
	...
	//################################# Function ###################################
	// Check การรองรับ Concurrency
	function isConcurrency(
		$parts_pocode = '', 
		$parts_received_details_array = ''
	){
	    ...
	    foreach($parts_received_details_array as $value){
			...
			while ($purchaseOrderPartsDetails_result = pg_fetch_array($purchaseOrderPartsDetails_query)) {
				...
				if($rcv_quantity > ($purchaseOrderPartsDetails_result["quantity"] - $rcv_quantity_count)){
					$isConcurrency++;
				}
			}
	...
	// ####################################### If is Not Concurrency ############################################
	elseif($isConcurrency == "isNotConcurrency"){
