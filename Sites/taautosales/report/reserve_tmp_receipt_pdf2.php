...
$qry_name=pg_query("SELECT * FROM \"V_repaircar\" WHERE res_id = '$res_id' ");
    if($res=pg_fetch_array($qry_name)){
        $car_id = $res['car_id'];
		$cus_id = $res['cus_id'];
		$cus_name = GetCusName($res['cus_id']);
        $license_plate = $res["license_plate"];
        $car_num = $res['car_num'];
        $mar_num = $res['mar_num'];
        $color = getCarColor($res['color']);
        $car_name = $res['name'];
		$product_id = $res['product_id'];
		$car_idno = $res['car_idno'];
		$wh_name = get_Warehouses($res['wh_id']);
		$rp_date = $res['rp_date'];
...
if($res_cus = pg_fetch_array($qry)){
        $cus_type=$res_cus['cus_type'];
        $branch_id=$res_cus['branch_id'];
		$address=$res_cus['address'];
		$add_post=$res_cus['add_post'];
		$telephone=$res_cus['telephone'];
		$card_type =$res_cus['card_type'];
		$card_id =$res_cus['card_id'];
		$contract_add=$res_cus['contract_add'];
		$contract_post=$res_cus['contract_post'];		
}
...
$save_data[$m] .='
	<span style="font-weight:bold; font-size:larger; text-align:right"><b>'.$page_name.'ใบเสร็จรับเงิน  </b></span>';

$save_data[$m] .='
<br><br>
<table  border="0" width="100%" style="font-size:smaller;">
	<tr>
		<td width="75%" style="font-weight:bold; font-size:small; text-align:left"></td>
		<td width="25%" style="font-weight:bold; font-size:small; text-align:left"><b>วันที่ใบเสร็จ &nbsp;&nbsp;'.$res_recdate['receipt_date'].'</b></td>
	</tr>
	<tr>
		<td width="75%" style="font-weight:bold; font-size:small; text-align:left"><b>เลขที่ฝากซ่อม  &nbsp;&nbsp;'.$res_id.'</b></td>';
		$save_data[$m] .='
...
<table cellpadding="3" cellspacing="0" border="1" width="100%" style="font-size:smaller;" >
	<tr>
		...
		<td width="35%">'.$cus_name.'&nbsp;&nbsp;</td>';
		if($branch_id == '0'){$str_branch = 'สำนักงานใหญ่';}else{$str_branch = 'สาขาที่ &nbsp;&nbsp;'.$branch_id;}
	
		$save_data[$m]  .='
		<td width="15%">ประเภทบัตร</td>';
		if($card_type == 'บัตรผู้เสียภาษีอากร'){$card_type = 'บัตรประจำตัวผู้เสียภาษีอากร';}else{$card_type = $card_type;}
		
		$save_data[$m] .='
		<td width="35%"> '.$card_type.'<br> เลขที่ '.$card_id.'</td>
	</tr>
	
	<tr>';
		if( $cus_type == 2 ){$str_address = 'ภ.พ. 20';}else{$str_address = 'บัตรประชาขน';}
	$save_data[$m] .='
		<td>ที่อยู่ตาม<br>'.$str_address.'</td>
		<td>'.$address.'&nbsp;'.$add_post.'<br>เบอร์โทรศัพท์  &nbsp;&nbsp;'.$telephone.'</td>
		<td>ที่อยู่ที่ติดต่อได้</td>
		<td>'.$contract_add.'&nbsp;'.$contract_post.'  </td>
	</tr>
	<tr>
		<td>แบบรถ</td>
		<td>'.$car_name.'</td>
	...
	<tr>
		<td>ทะเบียนรถ</td>
		<td>'.$license_plate.'&nbsp;&nbsp; ปีรถ '.$car_year.'</td>
		<td>ทะเบียน STOCK</td>
		<td>'.$car_idno.'</td>
	</tr>
</table>';
