...
$page_title = "พิมพ์สำเนาหนังสือสัญญาจะซื้อจะขายรถยนต์";
?>
...
$("#txt_name").autocomplete({
	source: "../po/po_autocomplete.php?cmd=resid_print",
	minLength:1
});
...
$("#search").click(function(){
	if($('#txt_name').val() == ""){
		...
		//alert(txt_con );
		$('#div_list').load('list_sale_contract_for_reprint.php?res_id='+$('#txt_name').val());
	}
});
	...
	function show_dialog(doc_no){
		$('body').append('<div id="div_dialog"></div>');
		$('#div_dialog').load('reprint_reason.php?doc_no='+doc_no+'&c1=1');
		$('#div_dialog').dialog({
