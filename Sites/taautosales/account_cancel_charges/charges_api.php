...
$j = 0;
$qry = pg_query("SELECT * FROM \"Invoices_account\" WHERE \"is_print\" = '0' AND \"cancel\" = false ORDER BY \"maturity_date\" DESC, \"doer_stamp\" DESC ");
...
<script>
	function show_detail(inv_no)
	{
		...
		$('#div_print').load('frm_charges_cancel.php?inv_no='+inv_no);
		$('#div_print').dialog({
			title: 'ยกเลิก ตั้งหนี้ค่าใช้จ่ายอื่นๆ (บัญชี) : '+inv_no,
			resizable: false,
			modal: true,  
			width: 780,
			height: 620,
